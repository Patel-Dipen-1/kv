 ...
 * Must match backend utils/phoneUtils.js logic
 */
...
/**
 * Formats phone number to +91 format for storage/API
 * @param {string} phone - Phone number in any format
 ...
 */
export const get10DigitPhone = (storedPhone) => {
  if (!storedPhone) return null;
  return normalizePhone(storedPhone);
};

/**
 * Formats phone for display (removes +91 prefix)
 * @param {string} phone - Phone number in any format
 * @returns {string} - Display-friendly phone number
 */
